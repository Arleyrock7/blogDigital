...
class Widget_Video extends WP_Widget {
	public function __construct() {
			$widget_ops = array(
				...
				'description' => esc_html_x( 'A short description about you.', 'video widget description', 'revalia' )
			);
			$control_ops = array( 'id_base' => 'video-widget' );
			parent::__construct( 'video-widget', sprintf( esc_html_x( '%s - video', 'video widget name', 'revalia' ), REVALIA_THEME_NAME ), $widget_ops, $control_ops );
	...
	public function widget( $args, $instance ) {
		...
		echo wp_kses_post( $args['before_widget'] );
			...
			if ( $embed_code ):
				...
				echo '<div class="post-iframe"><iframe '.$width.' height="'.$height.'" src="'.$type.'"></iframe></div>';
			endif;
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'embed_code' ); ?>"><?php esc_html_e('Embed Code :' , 'revalia'); ?></label>
		...
		<br></br>
		<p>
			<label for="<?php echo $this->get_field_id( 'video_type' ); ?>"><?php esc_html_e('Video Type :' , 'revalia'); ?></label>
		...
		<p>
			...
			<br></br>
			Put here the video id : https://www.youtube.com/watch?v=oIlIVFBBbNw EX : "oIlIVFBBbNw"
		</p>
	<?php
	}
}
